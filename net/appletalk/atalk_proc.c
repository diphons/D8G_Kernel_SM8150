...
int __init atalk_proc_init(void)
{
	...
	p = proc_create_seq("interface", S_IRUGO, atalk_proc_dir,
			&atalk_seq_interface_ops);
	if (!p)
		goto out_interface;

	p = proc_create_seq("route", S_IRUGO, atalk_proc_dir,
			&atalk_seq_route_ops);
	if (!p)
		goto out_route;

	p = proc_create_seq("socket", S_IRUGO, atalk_proc_dir,
			&atalk_seq_socket_ops);
	if (!p)
		goto out_socket;

	p = proc_create_seq_private("arp", S_IRUGO, atalk_proc_dir, &aarp_seq_ops,
