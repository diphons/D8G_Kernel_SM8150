...
int __init x25_proc_init(void)
{
	...
	if (!proc_create_seq("x25/route", S_IRUGO, init_net.proc_net,
			&x25_seq_route_ops))
		goto out;

	if (!proc_create_seq("x25/socket", S_IRUGO, init_net.proc_net,
			&x25_seq_socket_ops))
		goto out;

	if (!proc_create_seq("x25/forward", S_IRUGO, init_net.proc_net,
			&x25_seq_forward_ops))
		goto out;
	return 0;
